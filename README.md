...
2. **Bucket side (CI).** [.github/workflows/autoupdate.yml](.github/workflows/autoupdate.yml)
   runs [scripts/autoupdate.py](scripts/autoupdate.py) every 6 hours (plus
   manual dispatch). The script takes the latest *stable* upstream release
   (`/releases/latest` never returns beta/rc prereleases), reads SHA-256
   digests from the GitHub asset metadata — it never downloads the binaries —
   and commits `version`/`hash` bumps back to this repo, so fresh installs
   always resolve to the current stable build.
